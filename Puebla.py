...
class LTR(QMainWindow):
    ...
    def initGui(self):
        ...
        self.ui_main.initBox_Cliente.addItems(["HBPO", "CKD"])
        ...
        match self.state:
            ...
            case 2:
                ...
                if self.mStatus_Server == 1 and self.Servercontrol == True:
                    ...
                    set_Register(chamber, self.PartNo, serial_final, leak, 7)
                ...
                if int(self.PzsFaltantes) == 0 and int(self.PzsRealizadas) == int(self.PzsTotales):
                    ...
                    elif current_client == "HBPO":
                        # Envia etiqueta HBPO
                        Print_HBPO(currDate_HBPO, partNo, Qty, internal_PartNo, serial)

                    elif current_client == "CKD":
                        # Envia etiqueta CKD
                        Print_CKD(currDate_CKD, partNo, Qty, internal_PartNo, serial)
        ...
        if result == True:
            if self.Key ==True:
                ...
                if CLIENT == "MERIDA":
                    ...
                    QMessageBox.information(None, "Etiqueta Interna Impresa", "Se ha enviado la etiqueta correctamente")
                    ...
                    Print_HBPO(currDate, PartNo, Qty, internal_PartNo, Serial, 1)
                    QMessageBox.information(None, "Etiqueta HBPO Impresa", "Se ha enviado la etiqueta correctamente")

                elif CLIENT == "CKD":
                    currDate = datetime.now()
                    currDate = currDate.strftime("%d.%m.%y")
                    internal_PartNo = self.Get_internalPartNo(PartNo)
                    Print_CKD(currDate, PartNo, Qty, internal_PartNo, Serial, 1)
                    QMessageBox.information(None, "Etiqueta CKD Impresa", "Se ha enviado la etiqueta correctamente")
